"""
Hnswlib-based vector store for efficient similarity search.
...
from sentrylens.core.exceptions import VectorStoreError
...
class HnswlibVectorStore:
    """
    Hnswlib-based vector store for error embeddings.
    ...
    def __init__(
        ...
        index_type: str = "hnsw"  # Kept for backwards compatibility
    ):
        """
        ...
        Args:
            ...
            index_type: Ignored, kept for backwards compatibility
        """
        self.dimension = dimension or settings.EMBEDDING_DIMENSION
        self.index_type = "hnswlib"

        logger.info(
            "Initializing Hnswlib VectorStore",
            dimension=self.dimension
        )

        # Initialize Hnswlib index (max_elements=100k should be plenty)
    ...
    def _create_index(self) -> hnswlib.Index:
        """
        Create Hnswlib index.

        Returns:
            Initialized Hnswlib index
        """
        try:
            index = hnswlib.Index(space='cosine', dim=self.dimension)
            index.init_index(max_elements=100000, ef_construction=200, M=16)
            logger.info("Created Hnswlib index with cosine space")
            return index

        except Exception as e:
            raise VectorStoreError(f"Failed to create Hnswlib index: {e}")
    ...
    def add_embeddings(
        ...
        try:
            ...
            # Hnswlib handles normalization internally for cosine space
            # Add embeddings with their indices
    ...
    def search(
        ...
        Returns:
            List of (error_id, similarity) tuples, sorted by similarity (higher is better)
        """
        ...
        try:
            ...
            # knn_query returns (labels, distances) where distances are in cosine space (0-2)
            # For cosine space: distance = 1 - cosine_similarity
            # So similarity = 1 - distance (but Hnswlib may return negative distances for cosine)
            num_results = min(top_k, len(self.error_ids))
            indices, distances = self.index.knn_query(query_array, k=num_results)

            # Convert results
            results = []
            for idx, dist in zip(indices[0], distances[0]):
                if 0 <= idx < len(self.error_ids):  # Valid index
                    error_id = self.error_ids[int(idx)]

                    # Convert cosine distance to similarity score (0-1)
                    # Hnswlib returns cosine distance (0-2 range)
    ...
    def save(self, filepath: Optional[Path] = None) -> Path:
        ...
        if filepath is None:
            ...
            filepath = settings.INDEXES_DIR / f"hnswlib_index_{timestamp}"
        ...
        try:
            # Save Hnswlib index
            index_path = filepath / "index.hnsw"
    ...
    @classmethod
    def load(cls, filepath: Path) -> 'HnswlibVectorStore':
        """
        ...
        Returns:
            Loaded HnswlibVectorStore instance
        """
        filepath = Path(filepath)
        ...
        try:
            ...
            store = cls(
                ...
                index_type=metadata.get('index_type', 'hnswlib')
            )
